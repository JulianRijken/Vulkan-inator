...
void SwapChain::CreateSwapChain(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, const glm::ivec2& extents)
{
    ...
    vulkanUtil::QueueFamilyIndices indices = vulkanUtil::FindQueueFamilies(physicalDevice);
...
void SwapChain::CreateImageViews()
{
    ...
    for(size_t i = 0; i < m_SwapChainImages.size(); i++)
    {
        m_SwapChainImageViews[i] =
            vulkanUtil::CreateImageView(m_SwapChainImages[i], m_SwapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
    }
}
